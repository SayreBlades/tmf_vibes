...
[Figure: State machine diagram illustrating the Product Order Lifecycle. States include Rejected, Acknowledged, Pending, InProgress, Assessing Cancellation, Held, Pending Cancellation, Cancelled, Completed, Partial, Failed. Transitions show conditions like 'start order business validation', 'order processing needs extra information', 'extra information cannot be supplied', 'order cancellation requested', 'order cancellation denied', 'order cancellation accepted', 'order cancellation failed', 'issue is solved', 'ordering process is blocked', 'issue cannot be solved', 'order completely treated', 'order partially completed/failed', 'order completely failed'.]
...
[Figure: State machine diagram illustrating the Cancel Product Order Lifecycle. States include acknowledged, InProgress, terminatedWithError, done. Transitions connect these states sequentially from acknowledged to done, with a possible transition from InProgress to terminatedWithError.]
...
[Figure: Diagram illustrating Use Case 1 (Acquisition). Shows relationships between Catalog (ProductOfferingId, ProductSpecificationId), ProductOrder (relatedParty, channel, orderItem), and specific product details (Mobile Telephony, TMF Tariff Plan, Coverage Options) linked with IDs and attributes.]
...
[Figure: Diagram illustrating Use Case 2 (Modification). Shows an Inventory state with ProductOfferingId, ProductSpecificationId, and productId linked to Mobile Telephony and Coverage. An arrow indicates a modification order changing Coverage from National to International, reflected in the productOrder with a 'change' action on the relevant orderItem.]
...
[Figure 1 - ProductOrder: UML class diagram showing the ProductOrder resource and its relationships with related entities like ProductOrderItem, RelatedPartyOrPartyRoleRef, BillingAccountRef, ExternalIdentifier, Note, OrderPrice, ProductOfferingQualificationRef, PaymentRef, QuoteRef, OrderRelationship, ProductOrderErrorMessage, ProductOrderJeopardyAlert, ProductOrderMilestone, RelatedChannel, and AgreementRef. Attributes for ProductOrder include id, href, cancellationDate, category, completionDate, description, etc. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 2 - GeographicAddress: UML class diagram showing the GeographicAddress resource and related entities like GeographicLocation, GeographicSubAddress, ExternalIdentifier, GeographicPoint, etc. Attributes for GeographicAddress include id, href, city, country, postcode, stateOrProvince, streetName, etc. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 3 - Intent: UML class diagram showing the Intent resource and its relationships with Characteristic, RelatedPartyOrPartyRoleRef, Attachment, EntityRef, and IntentExpression. Attributes for Intent include id, href, context, creationDate, description, isBundle, lastUpdate, etc. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 4 - Organization: UML class diagram showing the Organization entity (inheriting from Party) and its relationships with OrganizationParentRelationship, OrganizationChildRelationship, OrganizationIdentification, Attachment, OtherNameOrganization, and TimePeriod. Attributes include isLegalEntity, organizationType, tradingName, status, etc. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 5 - ProductOrderItem: UML class diagram showing the ProductOrderItem resource and its relationships with Product, OrderTerm, QuoteItemRef, BillingAccountRef, ProductOfferingQualificationItemRef, AppointmentRef, PaymentRef, OrderPrice, ProductOrderItemRelationship, and ItemActionType/State enumerations. Attributes include id, quantity, action, state, etc. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 6 - Characteristic: UML class diagram showing the base Characteristic class and its specializations: NumberArrayCharacteristic, IntegerCharacteristic, StringArrayCharacteristic, IntegerArrayCharacteristic, BooleanArrayCharacteristic, BooleanCharacteristic, FloatCharacteristic, FloatArrayCharacteristic, StringCharacteristic, ObjectCharacteristic, NumberCharacteristic, ObjectArrayCharacteristic. Attributes include id, name, value, valueType. Also shows CharacteristicRelationship. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 7 - Individual: UML class diagram showing the Individual entity (inheriting from Party) and its relationships with Disability, LanguageAbility, IndividualIdentification, Attachment, OtherNameIndividual, and related enumerations. Attributes include birthDate, deathDate, familyName, gender, givenName, maritalStatus, etc. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 8 - Party: UML class diagram showing the abstract Party entity and its concrete specializations Individual and Organization. Shows relationships with ContactMedium, ExternalIdentifier, PartyCreditProfile, Characteristic, RelatedPartyOrPartyRole, TaxExemptionCertificate, PartyRole, and Attachment. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 9 - Product: UML class diagram showing the Product resource and its relationships with ProductRelationship, ProductSpecificationRef, BillingAccountRef, RelatedPlaceRefOrValue, ProductTerm, RealizingResource, RealizingService, ProductCharacteristic, ProductPrice, RelatedPartyOrPartyRole, AgreementItemRef, RelatedOrderItem, ProductOfferingRef, IntentRefOrValue. Attributes include id, href, name, description, productSerialNumber, status, etc. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]

[Figure 10 - PartyRole: UML class diagram showing the PartyRole resource and its relationships with AccountRef, AgreementRef, ContactMedium, CreditProfile, PaymentMethodRef, PartyRoleSpecificationRef, RelatedPartyOrPartyRole, Characteristic. Concrete specializations like BusinessPartner, Consumer, Producer, Supplier are shown. Attributes include id, href, name, status, engagedParty, etc. Legend indicates color coding for Resource, Sub-resource, and Mandatory property.]
...
[Figure 11 - PartyOrPartyRole: UML class diagram illustrating the concept of referring to either a Party (Individual or Organization) or a PartyRole (BusinessPartner, Consumer, Producer, Supplier) using a common reference structure. Shows relationships between Party, PartyRole, and potentially a reference class.]
...
[Figure 12 - CancelProductOrder: UML class diagram showing the CancelProductOrder resource and its relationship with ProductOrderRef and the TaskStateType enumeration. Attributes include cancellationReason, creationDate, effectiveCancellationDate, href, id, requestedCancellationDate, state. Legend indicates color coding for Resource, Sub-resource, Enumeration and Mandatory property.]
...
[Figure 13 Notification Pattern: Diagram showing the generic structure for notifications. An 'Event' object contains common attributes (correlationId, description, domain, eventId, eventTime, fieldPath, href, id, priority, timeOccurred, title). It relates to a 'SpecificEvent', which relates to a 'SpecificEventPayload', which contains the 'SpecificResource' under a field named 'resourceName'.]
  ...
  "productOrderItem": [
   ...
   },
   {
    ...
    "payment": [
     {
      "id": "2365",
      "href": "https://host:port/paymentManagement/v5/