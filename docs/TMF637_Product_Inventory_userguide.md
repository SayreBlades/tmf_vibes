...
[Figure: UML Class Diagram for 'Product'. This diagram shows the `Product` class at the center with its attributes (e.g., `id`, `href`, `name`, `description`, `creationDate`, `orderDate`, `startDate`, `terminationDate`, `status`, `isBundle`, `isCustomerVisible`, `productSerialNumber`) and its various relationships. Key relationships include: `productCharacteristic` (to `Characteristic`), `billingAccount` (to `BillingAccountRef`), `productRelationship` (to `ProductRelationship`), `productTerm` (to `ProductTerm`), `productOffering` (to `ProductOfferingRef`), `productOfferingPrice` (to `ProductOfferingPriceRef`), `productOrderItem` (to `RelatedOrderItem`), `productSpecification` (to `ProductSpecificationRef`), `relatedParty` (to `RelatedPartyOrPartyRole`), `realizingResource` (to `ResourceRef`), `realizingService` (to `ServiceRef`), `place` (to `RelatedPlaceRefOrValue`), `intent` (to `IntentRefOrValue`), `agreementItem` (to `AgreementItemRef`), and `targetProductSchema` (to `TargetProductSchema`). Several related classes like `Price`, `PriceAlteration`, `ProductPrice`, `Characteristic`, `ProductStatusType` (enum), `ItemActionType` (enum) are also depicted with their attributes and interconnections. Cardinalities are indicated on the relationships (e.g., 0..*, 0..1). A legend indicates the meaning of different box colors/styles (Resource entry point, Sub-resource, Reference entity, Enumeration, Sub-resource with details in separate diagram, Mandatory property marker).]
...
[Figure: UML Class Diagram for 'GeographicAddress'. This diagram focuses on the `GeographicAddress` class and its components. `GeographicAddress` has attributes like `city`, `country`, `postcode`, `streetName`, `streetNr`, etc. It relates to `GeographicSubAddress` (0..*), `GeographicLocationRefOrValue` (0..1), `RelatedPartyRefOrPartyRoleRef` (0..*), and `ExternalIdentifier` (0..*). `GeographicSubAddress` has attributes like `buildingName`, `levelNumber`, `levelType` and relates to `GeographicSubAddressUnit` (0..*). The `Place` class acts as a discriminator for `GeographicAddress`, `GeographicLocation`, and `GeographicSite`. Relationships are shown to `StandardIdentifier`, `GeographicSite`, `GeographicSiteRelationship`, `CalendarPeriod`, `HourPeriod`, `PartyRef`, and `PartyRoleRef`. A legend explains the color coding for different types of sub-resources.]
...
[Figure: UML Class Diagram for 'Intent'. This diagram shows the `Intent` class with attributes like `id`, `href`, `name`, `description`, `context`, `creationDate`, `lastUpdate`, `lifecycleStatus`, `priority`, `validFor`, `isBundle`, `version`. `Intent` relates to `IntentExpression` (0..1), `Characteristic` (0..*), `EntityRelationship` (0..* representing `intentRelationship`), `EntityRef` (0..1 representing `intentSpecification`), `RelatedPartyRefOrPartyRoleRef` (0..* representing `relatedParty`), and `AttachmentRefOrValue` (0..* representing `attachment`). Related classes like `EntityRelationship`, `EntityRef`, `RelatedPartyRefOrPartyRoleRef`, `AttachmentRefOrValue`, `PartyRefOrPartyRoleRef`, `PartyRef`, `PartyRoleRef`, `Attachment`, and `AttachmentRef` are also shown with their attributes and relationships. A legend explains the color coding and mandatory properties.]
...
[Figure: UML Class Diagram for 'Characteristic'. This diagram illustrates the inheritance structure for `Characteristic`. The base abstract class `Characteristic` has attributes `id`, `name`, `valueType`, `@baseType`, `@schemaLocation`, `@type`, and a relationship `characteristicRelationship` to `CharacteristicRelationship`. Several concrete classes inherit from `Characteristic`, each defining a specific `value` attribute type: `IntegerCharacteristic` (Integer), `StringArrayCharacteristic` (String[0..*]), `IntegerArrayCharacteristic` (Integer[0..*]), `StringCharacteristic` (String), `NumberCharacteristic` (Number), `ObjectCharacteristic` (object), `BooleanArrayCharacteristic` (Boolean[0..*]), `NumberArrayCharacteristic` (Number[0..*]), `BooleanCharacteristic` (Boolean), `ObjectArrayCharacteristic` (object[0..*]). A legend indicates sub-graphs and mandatory properties.]
...
[Figure: Notification Pattern. Shows a hierarchy: A base `Event` class has attributes like `correlationId`, `description`, `domain`, `eventId`, `eventTime`, `eventType`, etc. A `SpecificEvent` class inherits from `Event` and has a relationship named `event` (cardinality 1) to a `SpecificEventPayload` class. The `SpecificEventPayload` class has a relationship named `resourceName` (cardinality 1) to a `SpecificResource` class.]
